...
namespace InventorySystemCsharp
{
    public partial class Admin_home : Form
    {
        ...
        private void Mregister_btn_Click(object sender, EventArgs e)
        {
            if (MfnameTxt.Text != "" && MlnameTxt.Text != "" && MusernameTxt.Text != "" && MphonenumTxt.Text != "" && MpassTxt.Text != "" && MrepassTxt.Text != "" && typecomboTxt.Text != "")
            {
                if (MpassTxt.Text == MrepassTxt.Text)
                {
                    try
                    {
                        // Assuming you're using MySql.Data.MySqlClient namespace for MySQL operations
                        //string connectionString = @"data source=(local);initial catalog=IMS;user id=sa;password=Indigo@123;";
                        string query = "INSERT INTO [dbo].[users] (first, last, username , password, phone, usertype) " +
                            "VALUES ( '"+ MfnameTxt.Text +"', '"+ MlnameTxt.Text +"' , '"+ MusernameTxt.Text +"' , '"+ MpassTxt.Text +"' ," +
                            " LEFT('"+ MphonenumTxt.Text +"', 15), '"+ typecomboTxt.Text +"' )";

                        using (SqlConnection myConn = new SqlConnection(@"data source=(local);initial catalog=IMS;user id=sa;password=Indigo@123;"))
                        {
                            myConn.Open(); // Open the connection before executing the command
        ...
        private void MphoneTxt_Click(object sender, EventArgs e)
        {
