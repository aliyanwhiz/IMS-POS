...
namespace InventorySystemCsharp
{
    public partial class Manager_home : Form
    {
        ...
        private void additem_Click(object sender, EventArgs e)
        {
            if(model.Text!="" && part.Text!="" && comboBox1.Text!="" && price.Text!="" && instock.Text != "")
            {
                try
                {
                    ...
                    string query = "INSERT INTO [Products] ([model], [part], [type], [price], [instock], [isDeleted]) VALUES('" + Convert.ToString(model.Text.Trim()) +"', '"+ Convert.ToString(part.Text.Trim()) + "', '"+ Convert.ToString(comboBox1.Text.Trim()) +"', "+ Convert.ToInt32(price.Text.Trim()) +", "+ Convert.ToInt32(instock.Text.Trim()) + ", 0); ";
        ...
        void FillGridView()
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select * from Products WHERE isDeleted=0", myConn);
        ...
        private void u_itemBtn_Click(object sender, EventArgs e)
        {
            if (u_modelTxt.Text != "" && u_partTxt.Text != "" && u_typeCombo.Text != "" && u_priceTxt.Text != "" && u_stockTxt.Text != "")
            {
                ...
                {
                    string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
                    //string query = "update `Products` set `model`= '" + u_modelTxt.Text + "',`part`= '" + u_partTxt.Text + "',`type`= '" + u_typeCombo.Text + "',`price`='" + u_priceTxt.Text + "', `instock`= '" + u_stockTxt.Text + "'where `id`= '"+ u_itemcodeTxt.Text +"' ";
                    string query = "UPDATE [Products]  SET[model] = '" + Convert.ToString(u_modelTxt.Text.Trim()) + "', [part] = '" + Convert.ToString(u_partTxt.Text.Trim()) + "', [type] = '" + Convert.ToString(u_typeCombo.Text.Trim()) + "', [price] = " + Convert.ToInt32(u_priceTxt.Text.Trim()) + ",[instock] = " + Convert.ToInt32(u_stockTxt.Text.Trim()) + " WHERE[id] = " + u_itemcodeTxt.Text.Trim() + "; ";
                    SqlCommand cmd = new SqlCommand(query, myConn);
                    myConn.Open();
                    cmd.ExecuteNonQuery();
                    myConn.Close();
        ...
        {
            string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
            SqlDataAdapter u_sda = new SqlDataAdapter("select * from Products WHERE isDeleted=0", myConn);
        ...
        private void del_item_btn_Click(object sender, EventArgs e)
        {
            if (d_modelTxt.Text != "" && d_partTxt.Text != "" && d_typeCombo.Text != "" && d_priceTxt.Text != "" && d_instockTxt.Text != "")
            {
                ...
                {
                    string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
                    string query = "UPDATE Products SET isDeleted = 1 where id =" + Convert.ToInt32(d_itemcodeTxt.Text.Trim());
                    SqlCommand cmd = new SqlCommand(query, myConn);
                    myConn.Open();
                    cmd.ExecuteNonQuery();
                    myConn.Close();
        ...
        {
            string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
            SqlDataAdapter d_sda = new SqlDataAdapter("select * from Products WHERE isDeleted=0", myConn);
        ...
        private void make_unPaid_btn_Click(object sender, EventArgs e)
        {
            if (p_order_idTxt.Text != ""&& p_order_detailsTxt.Text !="")
            {
                ...
                {
                    string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
                    //string query = "update `orders` set `paid`= 0 where `id`= '" + p_order_idTxt.Text + "' ";
                    string query = "update orders set isPaid = 0 where id=" + p_order_idTxt.Text +"";
                    SqlCommand cmd = new SqlCommand(query, myConn);
                    myConn.Open();
                    cmd.ExecuteNonQuery();
                    myConn.Close();
            ...
            {
                MessageBox.Show("please select an order to make changes");
            }
        }
        ...
        {
            string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
            string query = "select * from orders where isPaid = 1 AND isDeleted = 0 ";
            SqlDataAdapter p_sda = new SqlDataAdapter(query, myConn);
        ...
        private void cancel_order_btn_Click(object sender, EventArgs e)
        {
            if (p_order_idTxt.Text != ""&& p_order_detailsTxt.Text !="")
            {
                ...
                {
                    string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
                    string query = "update orders set isDeleted = 1 where id="+ p_order_idTxt.Text;
                    SqlCommand cmd = new SqlCommand(query, myConn);
                    myConn.Open();
                    cmd.ExecuteNonQuery();
                    myConn.Close();
        ...
        private void unp_make_btn_Click(object sender, EventArgs e)
        {
            if (unp_orderidTxt.Text != "" && unp_orderdetailsTxt.Text !="")
            {
                try
                {
                    ...
                    string query = "UPDATE orders SET isPaid = 1 WHERE id =  " + unp_orderidTxt.Text + " ";
                    SqlCommand cmd = new SqlCommand(query, myConn);
                    myConn.Open();
                    cmd.ExecuteNonQuery();
                    myConn.Close();
        ...
        {
            string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
            string query = "select * from orders where isPaid = 0 AND isDeleted = 0";
            SqlDataAdapter unp_sda = new SqlDataAdapter( query, myConn);
        ...
        private void unp_cancelorder_btn_Click(object sender, EventArgs e)
        {
            if (unp_orderidTxt.Text != "" && unp_orderdetailsTxt.Text != "")
            {
                ...
                {
                    string connectionString = ConfigurationManager.ConnectionStrings["ConStr"].ConnectionString;SqlConnection myConn = new SqlConnection(connectionString);
                    string query = "update orders set isDeleted = 1 where id=" + unp_orderidTxt.Text ;
                    SqlCommand cmd = new SqlCommand(query, myConn);
                    myConn.Open();
                    cmd.ExecuteNonQuery();
                    myConn.Close();
